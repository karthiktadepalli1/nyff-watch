...
- **History:** stores compact changes across all festival screenings on the `data` branch, including observation times, cache headers, availability, rush status, and screening details. A report is generated daily and on demand. New shows enter the dataset; alerts target the four screenings above.
- **Health:** Healthchecks.io independently notices 30 minutes without successful check-ins. Three consecutive failures trigger an earlier failure alert. Feed and page checks have separate health status. Persistent notification failures also affect the feed's health check.
